...
exports.registerEntryHandlers = registerEntryHandlers;
// @ts-ignore
const index_cjs_1 = require("../db/index.cjs");
function registerEntryHandlers(ipcMain) {
    // エントリ一覧
    ipcMain.handle('entries:list', async (_, params) => {
    ...
    // エントリ取得
    ipcMain.handle('entries:get', async (_, { id }) => {
        const db = (0, index_cjs_1.getDatabase)();
    ...
    // エントリ作成/更新
    ipcMain.handle('entries:upsert', async (_, params) => {
        const db = (0, index_cjs_1.getDatabase)();
        const { id: existingId, project_id, title, body_markdown, source_json, is_starred = false, is_locked = false, tag_ids = [] } = params;
        const id = existingId || (0, index_cjs_1.generateId)();
        ...
        const upsertEntry = db.transaction(() => {
          ...
          SET title = ?,
              ...
              updated_at = ?
          WHERE id = ?
        `).run(title || null, body_markdown, source_json || null, is_starred ? 1 : 0, is_locked ? 1 : 0, timestamp, id);
            }
            else {
                // 新規作成
                db.prepare(`
        ...
        `).run(id, project_id, title || null, body_markdown, source_json || null, is_starred ? 1 : 0, is_locked ? 1 : 0, timestamp, timestamp);
            }
    ...
    // スター切り替え
    ipcMain.handle('entries:toggleStar', async (_, { id, is_starred }) => {
        const db = (0, index_cjs_1.getDatabase)();
        db.prepare(`
      UPDATE prompt_entries
      SET is_starred = ?
    ...
    // ロック切り替え
    ipcMain.handle('entries:toggleLock', async (_, { id, is_locked }) => {
        const db = (0, index_cjs_1.getDatabase)();
        db.prepare(`
      UPDATE prompt_entries
      SET is_locked = ?
